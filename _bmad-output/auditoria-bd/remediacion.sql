...
-- Confirmado por el catálogo (pg_constraint/pg_index) y por la verificación
-- de datos: estas 5 columnas FK no son la primera columna de ningún índice.
-- Nota: la 5ª (docente_secciones.seccion_id) se omitió en el informe
-- original; fue detectada por la query del catálogo en verificacion-datos.sql.

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_usuario_roles_rol_id
    ON usuario_roles(rol_id);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_docente_secciones_seccion_id
    ON docente_secciones(seccion_id);
...
ALTER TABLE consentimientos
    ...
    CHECK (fuente IS NULL OR fuente IN ('SIE_LOCAL', 'MINEDU', 'IMPORTACION', 'WEB', 'LOPDP'));
