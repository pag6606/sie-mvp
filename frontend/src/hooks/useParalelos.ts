...
export function useParalelos(periodoId: string) {
  return useQuery<Paralelo[]>({
    queryKey: ['paralelos', periodoId],
    queryFn: () => api.get(`/paralelos?periodoId=${periodoId}&size=200`).then(r => {
      return normalizePageResponse<Paralelo>(r.data).content
    }),
    staleTime: 5 * 60 * 1000,
    gcTime: 30 * 60 * 1000,
    enabled: !!periodoId,
...
export function useParalelosPaginadas(periodoId: string) {
  const [page, setPage] = useState(0)

  const query = useQuery<PaginatedResponse<Paralelo>>({
    queryKey: ['paralelos', periodoId, page],
    queryFn: () => api.get(`/paralelos?periodoId=${periodoId}&page=${page}&size=25`).then(r => {
      return normalizePageResponse<Paralelo>(r.data)
    }),
